...
//		}
//		readResidualLuma(0, 0, 0, 0, 0, 0);
//		int intra16x16DCLevel = i16x16DClevel;
//		int intra16x16ACLevel = i16x16AClevel;
//		int lumaLevel4x4 = level4x4;
//		int lumaLevel8x8 = level8x8;
//
//		if (chromaArrayType == 1 || chromaArrayType == 2) {
//			int NumC8x8 = 4 / (getSubWidthC() * getSubHeightC());
...
//			coeffLevel[i] = 0;
//		}
//		_coeff_token = bitBufferReader.readCE();
//		if (TotalCoeff(_coeff_token) > 0) {
//			if (TotalCoeff(_coeff_token) > 10 && TrailingOnes(_coeff_token) < 3) {
//				suffixLength = 1;
//			} else {
//				suffixLength = 0;
//			}
//		
//			for (int i = 0; i < TotalCoeff(_coeff_token); i++) {
//				if (i < TrailingOnes(_coeff_token)) {
//					_trailing_ones_sign_flag = (int) bitBufferReader.readNBits(1);
//					levelVal[i] = 1 - 2 * _trailing_ones_sign_flag
//				} else {
//					_level_prefix = bitBufferReader.readCE();
//					levelCode = (Math.min(15, _level_prefix) << suffixLength);
//					if (suffixLength > 0 || level_prefix >= 14) {
//						_level_suffix = (int) bitBufferReader.readNBits(v);
//						levelCode += level_suffix
//					}
//					if (_level_prefix >= 15 && suffixLength == 0) {
//						levelCode += 15;
//					}
//					if( level_prefix >= 16 ) {
//						levelCode += (1 << (_level_prefix - 3)) - 4096;
//					}
//					if (i == TrailingOnes(_coeff_token) && TrailingOnes(_coeff_token ) < 3) {
//						levelCode += 2
//					}
//					if (levelCode % 2 == 0) {
...
//			}
//			
//			if (TotalCoeff(_coeff_token) < endIdx - startIdx + 1) {
//				_total_zeros = bitBufferReader.readCE();
//				zerosLeft = _total_zeros
//			} else {
//				zerosLeft = 0;
//			}
//			
...
//				if (zerosLeft > 0) {
//					_run_before = bitBufferReader.readCE();
//					runVal[i] = run_before;
//				} else {
//					runVal[i] = 0;
//				}
...
//			}
//			runVal[TotalCoeff(_coeff_token) - 1] = zerosLeft;
//			coeffNum = -1;
//			for (int i = TotalCoeff(_coeff_token) - 1; i >= 0; i--) {
//				coeffNum += runVal[i] + 1;
